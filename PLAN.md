...
- [x] Post-Phase-4 hardening while waiting on hardware: an independent code review and an independent security review of the full `core/`/`transport/`/`gateway/`/`node/` tree turned up five real bugs, all fixed, all covered by new host-native tests where the fix lives in `core/` (130 tests total, up from 127; two fixes are orchestration-layer and follow this repo's established pattern of being documented rather than host-tested — see `gateway/README.md`/`node/README.md`). Two were security-significant: JOIN_ACK carried no authentication at all, so any frame shaped like one (including a single broadcast) could hijack a node's gateway MAC, even an already-paired one; and `handle_cmd` never checked a CMD frame's sender against the paired gateway, so any radio on the channel could get a forged command executed. Both fixed together (D-015, DECISIONS.md): a token echo on JOIN_ACK, `NodePairingState::on_join_ack()` refusing to re-pair an already-paired node outright, and a `mac_equal()` check before executing any CMD. Also fixed: the gateway's `cmd_id` counter restarting at 0 every reboot could collide with a node's cached last-executed cmd_id and cause a new command to be silently skipped as a false duplicate (D-016, seeded from `esp_random()` now); `core/pl_fields.h`'s `FieldReader` silently accepted an unrecognized field-type byte as a zero-length field instead of rejecting it, desyncing the rest of the payload; and per-device `LastSeenSeq`/`SeqLossTracker` state wasn't reset on rejoin, permanently corrupting a rejoined node's published `loss_rate` after any reboot. All five fixed in `gateway/gateway.ino`, `node/node.ino`, and `wokwi/single-board/combined/combined.ino` alike (the Wokwi project is a from-scratch rewrite of the same logic, not a copy, so it needed each fix applied separately — same lesson as the field-mapping bug above).
- [x] Series articles drafted for Parts 1-3 (`articles/part1-esp-now-fundamentals.md`, `part2-two-boards-talking.md`, `part3-channel-problem-pairing.md`) — Part 3's draft was updated in the same pass to reflect the JOIN_ACK authentication fix above so the article matches shipped code, not a stale wire format.
